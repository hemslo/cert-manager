...
func ValidateACMEIssuerDNS01Config(iss *v1alpha1.ACMEIssuerDNS01Config, fldPath *field.Path) field.ErrorList {
	...
	for i, p := range iss.Providers {
		...
		if p.DNSPod != nil {
			if numProviders > 0 {
				el = append(el, field.Forbidden(fldPath.Child("dnspod"), "may not specify more than one provider type"))
			} else {
				numProviders++
				el = append(el, ValidateSecretKeySelector(&p.DNSPod.APIKey, fldPath.Child("dnspod", "apiKeySecretRef"))...)
			}
		}
		if numProviders == 0 {
